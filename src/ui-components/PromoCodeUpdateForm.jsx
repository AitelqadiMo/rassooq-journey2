...
export default function PromoCodeUpdateForm(props) {
  const {
    id: idProp,
    promoCode: promoCodeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = promoCodeRecord
      ? { ...initialValues, ...promoCodeRecord }
      : initialValues;
    setCode(cleanValues.code);
    setDescription(cleanValues.description);
    setDiscountType(cleanValues.discountType);
    setDiscountValue(cleanValues.discountValue);
    setMinPurchaseAmount(cleanValues.minPurchaseAmount);
    setMaxDiscountAmount(cleanValues.maxDiscountAmount);
    setUsageLimit(cleanValues.usageLimit);
    setUsageLimitPerUser(cleanValues.usageLimitPerUser);
    setValidFrom(cleanValues.validFrom);
    setValidUntil(cleanValues.validUntil);
    setIsActive(cleanValues.isActive);
    setApplicableToCategories(cleanValues.applicableToCategories ?? []);
    setCurrentApplicableToCategoriesValue("");
    setApplicableToProducts(cleanValues.applicableToProducts ?? []);
    setCurrentApplicableToProductsValue("");
    setIsPlusMembersOnly(cleanValues.isPlusMembersOnly);
    setIsFirstOrderOnly(cleanValues.isFirstOrderOnly);
    setTimesUsed(cleanValues.timesUsed);
    setCreatedAt(cleanValues.createdAt);
    setUpdatedAt(cleanValues.updatedAt);
    setErrors({});
  };
  const [promoCodeRecord, setPromoCodeRecord] =
    React.useState(promoCodeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getPromoCode.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPromoCode
        : promoCodeModelProp;
      setPromoCodeRecord(record);
    };
    queryData();
  }, [idProp, promoCodeModelProp]);
  React.useEffect(resetStateValues, [promoCodeRecord]);
  const [
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          validUntil,
          isActive,
          applicableToCategories: applicableToCategories ?? null,
          applicableToProducts: applicableToProducts ?? null,
          isPlusMembersOnly: isPlusMembersOnly ?? null,
          isFirstOrderOnly: isFirstOrderOnly ?? null,
          timesUsed: timesUsed ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updatePromoCode.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "PromoCodeUpdateForm")}
      {...rest}
    >
      <TextField
        label="Code"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || promoCodeModelProp)}
